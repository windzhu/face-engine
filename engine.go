...
const (
	...
	OrientPriorityHigherExt = C.ASF_OP_0_HIGHER_EXT
...
//
// 如果调用初始化函数失败则返回一个错误
func NewFaceEngine(dm C.MUInt32, op, faceScale, maxFaceNum, combinedMask C.MInt32) (*FaceEngine, error) {
...
func GetActiveFileInfo() (ActiveFileInfo, error) {
	...
	defer C.free(unsafe.Pointer(asfActiveFileInfo.fileVersion))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.sdkVersion))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.sdkKey))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.appId))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.sdkType))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.platform))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.endTime))
	defer C.free(unsafe.Pointer(asfActiveFileInfo.startTime))
...
func (engine *FaceEngine) ASFDetectFaces(width, height int, format C.MInt32, imgData []byte) (*C.ASF_MultiFaceInfo, error) {
	...
	defer C.free(unsafe.Pointer(faceInfo.faceRect))
	defer C.free(unsafe.Pointer(faceInfo.faceOrient))
	defer C.free(unsafe.Pointer(faceInfo.faceID))
...
func (engine *FaceEngine) GetVersion() Version {
	info := C.ASFGetVersion(C.MHandle(unsafe.Pointer(engine.handle)))
	return Version{
...
// 单人脸特征提取
func (engine *FaceEngine) ASFFaceFeatureExtract(width, height int, format C.MInt32, imgData []byte, faceInfo *SingleFaceInfo) (asfFaceFeature *C.ASF_FaceFeature, ff FaceFeature, err error) {
	...
	defer C.free(unsafe.Pointer(asfFaceFeature.feature))
	...
	ff.Feature = byteArr
	ff.FeatureSize = length
	asfFaceFeature.feature = arr
	return asfFaceFeature, ff, err
}
...
func (engine *FaceEngine) FaceFeatureCompare(feature1, feature2 *C.ASF_FaceFeature) (float32, error) {
	...
	C.free(unsafe.Pointer(feature1.feature))
	C.free(unsafe.Pointer(feature2.feature))
	return confidenceLevel, nil
}

// 人脸特征比对
func (engine *FaceEngine) FaceFeatureCompare2(feature1 *C.ASF_FaceFeature, feature2Byte []byte) (float32, error) {
	var feature2 *C.ASF_FaceFeature
	feature2 = &C.ASF_FaceFeature{}
	length := len(feature2Byte)
	feature2.featureSize = C.MInt32(length)
	arr := (*C.MByte)(C.malloc(C.size_t(int32(feature2.featureSize))))
	ps := (*[1 << 28]C.MByte)(unsafe.Pointer(arr))[:length:length]
	for i := 0; i < len(ps); i++ {
		ps[i] = C.MByte(feature2Byte[i])
	}
	feature2.feature = arr
...
func (engine *FaceEngine) GetAge() (AgeInfo, error) {
	...
	defer C.free(unsafe.Pointer(asfAgeInfo.ageArray))
...
func (engine *FaceEngine) GetGender() (GenderInfo, error) {
	...
	defer C.free(unsafe.Pointer(asfGenderInfo.genderArray))
...
func (engine *FaceEngine) GetFace3DAngle() (Face3DAngle, error) {
	...
	defer C.free(unsafe.Pointer(asfFace3DAngle.roll))
	defer C.free(unsafe.Pointer(asfFace3DAngle.yaw))
	defer C.free(unsafe.Pointer(asfFace3DAngle.pitch))
	defer C.free(unsafe.Pointer(asfFace3DAngle.status))
...
func (engine *FaceEngine) GetLivenessScore() (LivenessInfo, error) {
	...
	defer C.free(unsafe.Pointer(asfLivenessInfo.isLive))
